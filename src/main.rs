...
fn type_cmd(args: &[&str], mut out: Box<dyn std::io::Write>, mut err: Box<dyn std::io::Write>) {
    ...
    for path in paths.iter() {
        match fs::read_dir(path) {
            ...
            Err(_err) => write!(err, "there was a problem reading directory {} check if the directory exists and rash has valid permissions to read it", path).unwrap(),
        }
    }
...
fn pwd_cmd(mut out: Box<dyn std::io::Write>, mut err: Box<dyn std::io::Write>) {
    match env::current_dir() {
        ...
        Err(_) => write!(err, "error: maybe the current directory is deleted or you don't have sufficient persmissions").unwrap()
    }
}
...
fn try_not_builtin_command(command: &str, args: &[&str], mut out: Box<dyn std::io::Write>, mut err: Box<dyn std::io::Write>) {
    ...
    writeln!(err, "{command}: command not found").unwrap();
}
